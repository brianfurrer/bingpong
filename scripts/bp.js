...
// Current version: 0.21.0-1196 (1/26/2016)
...
var DEFAULT_STATUS_TEXT = "Created by <a href=\"http://www.reddit.com/user/kiefferbp\" target=\"_blank\">/u/kiefferbp</a>. v0.21.0-1196 (ALPHA)";
...
var bphCompatibleVersions = ["1.3.13.65"];
var bphLatestVersion = "1.3.13.65";
...
function launchEmailForAccount(accountIndex) { 
	var logout;
	
	disableSettings(true);
	clearStatusTimeout();
	changeStatusText("<img src=\"loader.gif\" width=\"16\" height=\"16\"></img> Signing in as " + accountUsernames[accountIndex] + "...", "&nbsp;", "&nbsp;");
	
	logoutOfAccount(function () {
		logIntoAccount(accountUsernames[accountIndex], accountPasswords[accountIndex], function () { // log-in was successful, so open the dashboard
			chrome.runtime.sendMessage(bphExtensionID, {action: "openOutlook"}, function (response) { 
				enableSettings();
				changeStatusText(DEFAULT_STATUS_TEXT, "&nbsp;", "&nbsp;");
			});
		}, function () { // log-in failed after two attempts
			enableSettings();
			changeStatusText("There was is an issue logging into this account.", "Verify that your account is in good standing and try again.", "&nbsp;");
   			...
   			enableSettings();
			changeStatusText("There was is an issue logging into this account.", "This account is blocked and most likely needs SMS verification.", "&nbsp;");
   			...
   			enableSettings();
			changeStatusText("There was is an issue logging into this account.", "This account is banned.", "&nbsp;");
   		...
   		enableSettings();
   		changeStatusText("There was is an issue logging out of the previous account.", "This is a bug, and will need to be reported to get fixed.", "&nbsp;");
...
function launchDashboardForAccount(accountIndex) {
	var logout;
...
function launchCaptchaForAccount(accountIndex) {
	// to-do
}

function openDashboardForCaptcha(callback) { 
	chrome.runtime.sendMessage(bphExtensionID, {action: "openDashboardForCaptcha"}, function (response) {
		callback();
	});
}

function closeDashboardForCaptcha(callback) { 
	chrome.runtime.sendMessage(bphExtensionID, {action: "closeDashboardForCaptcha"}, function (response) {
		callback();
	});
}
...
function moveSearchCaptchaBack(callback) { 
	chrome.runtime.sendMessage(bphExtensionID, {action: "moveSearchCaptchaBack"}, function (response) {
		callback();
	});
}
...
function verifyLogin(username, password, callbackOnSuccess, callbackOnFailure, callbackOnBlocked, callbackOnBanned, callbackOnCaptcha) { 
	getDashboardContents(function () {
